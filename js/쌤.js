$(function() {
	...
	// gnb 활성화
	$gnbmnu.on('mouseenter', function(evt) {
		evt.preventDefault();

		nowIdx = $gnbmnu.index(this);
		$lnbmnu.eq(nowIdx).stop().slideDown();
	});
	...
	$(window).on('scroll', function() {
		...
		if ($(this).scrollTop() > logoBottomDistance) {
			//1520:css에서 설정된 #wrap의 폭, 30:css에서 설정된 nav의 left값
